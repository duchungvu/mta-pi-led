#!/usr/bin/env python3
"""Build station metadata JSON from GTFS stops and station route CSV."""

import csv
import json
from collections import defaultdict
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[2]
STOPS_PATH = PROJECT_ROOT / "data" / "gtfs_subway" / "stops.txt"
STATION_CSV_PATH = PROJECT_ROOT / "examples" / "MTA_Subway_Stations_20250330.csv"
OUTPUT_PATH = PROJECT_ROOT / "data" / "mta_stations.json"

# Read stops.txt
stations = {}
with STOPS_PATH.open("r", encoding="utf-8") as f:
    reader = csv.DictReader(f)
    for row in reader:
        # Only process parent stations (location_type == 1)
        if row["location_type"] == "1":
            station_id = row["stop_id"]
            stations[station_id] = {
                "name": row["stop_name"],
                "lines": [],
                "direction_codes": [],
            }
        # Add direction codes to parent stations
        elif row["parent_station"] in stations:
            stations[row["parent_station"]]["direction_codes"].append(row["stop_id"])

# Read station CSV to get line information
station_lines = defaultdict(set)
with STATION_CSV_PATH.open("r", encoding="utf-8") as f:
    reader = csv.DictReader(f)
    for row in reader:
        station_id = row["GTFS Stop ID"]
        lines = row["Daytime Routes"].split()
        if station_id in stations:
            station_lines[station_id].update(lines)

# Add lines to each station
for station_id, station_data in stations.items():
    station_data["lines"] = sorted(list(station_lines[station_id]))

# Write to JSON file
with OUTPUT_PATH.open("w", encoding="utf-8") as f:
    json.dump(stations, f, indent=2)

print(
    f"Station database created with {len(stations)} stations saved to {OUTPUT_PATH}"
)

